...
namespace Lesson_8
{
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            #region Checking if a Diretory Exists create
            
            string basePath = Path.Combine("." + Path.DirectorySeparatorChar, "Data");
            
            Console.WriteLine($"Before: {basePath}");
            ...
            //Directory.Delete(basePath, true); // CAUTION
            DirectoryInfo d = new DirectoryInfo(basePath);
            string lastWritedTime = d.LastWriteTimeUtc.ToString("yyyyMMdd_HHmmss");
            Console.WriteLine($"After: {lastWritedTime}");

            // Not recomended
            //string filePathNotDOThis = "C://myuser/myapp/bin/Debug/user.json";

            string fileExamplePath = Path.Combine(basePath, "demo.txt");
            File.WriteAllText(fileExamplePath, lastWritedTime);

            Console.WriteLine($"After: {basePath}");
            ...
            File.WriteAllText(fileExamplePath, "Hello, World!");
            ...
            // Read all text
            string content = File.ReadAllText(fileExamplePath);
            Console.WriteLine(content);

            // Read all lines
            string[] lines2 = File.ReadAllLines(fileExamplePath);
            ...
            File.AppendAllText(fileExamplePath, "\nAppended Line");

            Console.WriteLine(File.ReadAllText(fileExamplePath));
            ...
            int stringLength = 10; 
            string[] randomStrings = Utilities.String.RandomStrings(numberOfStrings, stringLength);
